...
function addItem() {
    ...
    if (itemName && quantity >= 0) {
        mainStore.push({ name: itemName, quantity: quantity });
...
function editMainStoreItem(index) {
    ...
    if (newName && newQuantity >= 0) {
        mainStore[index] = { name: newName, quantity: newQuantity };
        localStorage.setItem('mainStore', JSON.stringify(mainStore));
        renderMainStore();
        updateStockSummary();
    }
...
function transferToGodown() {
    ...
    if (item && quantity > 0 && item.quantity >= quantity) {
        ...
        if (godownItem) {
            ...
            godown.push({ name: itemName, quantity: quantity });
        }
        localStorage.setItem('mainStore', JSON.stringify(mainStore));
...
function editGodownItem(index) {
    ...
    if (newName && newQuantity >= 0) {
        godown[index] = { name: newName, quantity: newQuantity };
        localStorage.setItem('godown', JSON.stringify(godown));
        renderGodown();
        updateStockSummary();
    }
